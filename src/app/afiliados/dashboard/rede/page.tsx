...
function NetworkNode({ node, depth }: { node: AffNode; depth: number }) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden ${
      depth === 0 ? "border-emerald-500/20 bg-emerald-500/4"
      : depth === 1 ? "border-violet-500/15 bg-violet-500/3"
      : "border-white/5 bg-white/2"
    }`}>
      <div className="px-4 py-3 flex items-center justify-between gap-3 cursor-pointer select-none" onClick={() => has && setOpen(!open)}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {has && (
            <svg className={`w-3.5 h-3.5 text-slate-600 transition-transform duration-200 ${open ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function AfiliadosRedePage() {
  ...
  // Per-item approve rate and error — fixes shared-state bug
  const [approveRates, setApproveRates]   = useState<Record<string, string>>({});
  ...
  async function load() {
    try {
      ...
      setTree(netRes.tree ?? []);
      const pendingList: PendingAffiliate[] = Array.isArray(pendingRes) ? pendingRes : [];
      setPending(pendingList);

      // Initialize per-item rates: default = parent rate - 10%, capped at 1%
      if (meRes?.commissionRate && pendingList.length > 0) {
        const defaultRate = Math.max(1, Math.floor(meRes.commissionRate * 100) - 10);
        const initRates: Record<string, string> = {};
        pendingList.forEach((p) => { initRates[p.id] = String(defaultRate); });
        setApproveRates(initRates);
      }
  ...
  async function handleApprove(subId: string, action: "approve" | "reject") {
    ...
    setApproveErrors((prev) => ({ ...prev, [subId]: "" }));
    const rate = approveRates[subId];
    const r = await fetch("/api/afiliados/pending", {
      ...
      body: JSON.stringify({
        id: subId,
        action,
        commissionRate: action === "approve" ? Number(rate) : undefined,
      }),
    });
    const d = await r.json();
    if (!r.ok) {
      setApproveErrors((prev) => ({ ...prev, [subId]: d.error || "Erro ao processar" }));
    } else {
      load();
    }
    setApprovingId(null);
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-0.5">
          {activeSubs.length} afiliado{activeSubs.length !== 1 ? "s" : ""} direto{activeSubs.length !== 1 ? "s" : ""}
        </p>
      </div>

      {/* Aprovações pendentes */}
      {pending.length > 0 && (
        <div className="bg-amber-500/6 border border-amber-500/20 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-amber-500/15 flex items-center gap-3">
            <svg className="w-4 h-4 text-amber-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z"/>
            </svg>
            <span className="font-bold text-amber-400 text-sm">Aprovações pendentes</span>
            <span className="ml-auto text-[10px] bg-amber-500/20 text-amber-300 border border-amber-500/25 px-2 py-0.5 rounded-full font-bold tabular-nums">
              {pending.length}
            </span>
          </div>
          <div className="divide-y divide-amber-500/10">
            {pending.map((p) => {
              ...
              const rate = approveRates[p.id] ?? String(Math.max(1, myRate - 10));
              const err  = approveErrors[p.id];
              ...
              return (
                <div key={p.id} className="px-5 py-4 space-y-3">
                  <div className="flex items-center gap-3 flex-wrap">
                    <div className="w-9 h-9 bg-amber-500/10 rounded-full flex items-center justify-center flex-shrink-0 text-sm font-bold text-amber-300 border border-amber-500/20">
                      {p.name[0]}
                    </div>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {/* Per-item rate input */}
                      <div className="flex items-center gap-1 bg-[#080c14] border border-white/10 rounded-xl px-3 py-1.5">
                        <input
                          type="number"
                          value={rate}
                          onChange={(e) => setApproveRates((prev) => ({ ...prev, [p.id]: e.target.value }))}
                          min={1}
                          max={myRate - 1}
                          step={1}
                      ...
                      >
                        {isApproving ? <span className="inline-block w-3 h-3 border border-[#080c14] border-t-transparent rounded-full animate-spin" /> : "Aprovar"}
                      </button>
                      <button
                        onClick={() => handleApprove(p.id, "reject")}
                        disabled={isApproving}
                        className="bg-rose-500/12 hover:bg-rose-500/22 disabled:opacity-50 text-rose-400 font-bold text-xs px-3 py-1.5 rounded-xl transition-all border border-rose-500/20"
                      >
                        Rejeitar
                      </button>
                    </div>
                  ...
                  {err && (
                    <div className="flex items-center gap-2 bg-rose-500/8 border border-rose-500/20 rounded-xl px-3 py-2 text-xs text-rose-400">
      ...
      {canRecruit && (
        <div className="bg-violet-500/8 border border-violet-500/20 rounded-2xl p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <div className="font-semibold text-violet-400 text-sm">Link para recrutar afiliados</div>
          </div>
          <p className="text-slate-500 text-xs mb-3">Compartilhe para trazer novos afiliados para sua rede. Você controla a comissão deles.</p>
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono text-violet-300/60 bg-violet-500/6 border border-violet-500/15 px-3 py-2 rounded-xl truncate flex-1">{recruitLink}</span>
            <button
              onClick={() => navigator.clipboard.writeText(recruitLink)}
      ...
      <div className="bg-[#0d1117] border border-white/6 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-white/5 flex items-center justify-between gap-4">
          ...
          <div className="text-[10px] text-slate-600 bg-white/4 px-3 py-1.5 rounded-lg border border-white/6 flex-shrink-0">
        ...
        {activeSubs.length === 0 ? (
          <div className="text-center py-14 text-slate-600">
            <svg className="w-10 h-10 mx-auto mb-3 opacity-30" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.2} d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"/>
            </svg>
                      ...
                      {isEditing ? (
                        ...
                        <div className="flex items-center gap-2">
                          <div className="text-right">
                            ...
                            {spread > 0 && <div className="text-[10px] text-emerald-400">+{spread}% pra você</div>}
                          </div>
                          <button
      ...
      <div className="bg-[#0d1117] border border-white/6 rounded-2xl p-5">
        ...
        <div className="space-y-2 text-xs text-slate-400">
          <p>
            Você define a comissão dos afiliados que recruta. Se seu acordo é{" "}
            <strong className="text-white">{myRate}%</strong> e você define um afiliado em{" "}
            <strong className="text-white">{Math.max(0, myRate - 10)}%</strong>, os{" "}
            <strong className="text-emerald-400">10%</strong> de diferença vêm direto para você a cada perda dos jogadores dele.
          </p>
          <p className="text-slate-600">A comissão de um afiliado nunca pode ser igual ou maior que a sua própria taxa.</p>
        </div>
      </div>
    </div>
  );
