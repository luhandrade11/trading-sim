...
export default function DashboardPage() {
  ...
  // Memoized stats — only recomputed when trades change
  const stats = useMemo(() => computeStats(trades), [trades]);

  const currentPrice = prices[selectedAsset]?.price ?? 0;
  const change24h    = prices[selectedAsset]?.change24h ?? 0;

  // Active entries for the current asset's chart (entry price lines)
  ...
  }, [currentPrice, balance, amount, placing, selectedDuration]);
  ...
  const handleSettle = useCallback(async (id: string, exitPrice: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showNotif(
          settled.result === "WIN"
            ? `🏆 Ganhou +$${settled.profit.toFixed(2)} em ${asset}`
            : `💸 Perdeu -$${settled.amount.toFixed(2)} em ${asset}`,
  ...
  const isConnected     = !pricesStale && lastUpdateTime !== null;
  ...
  return (
    <div className="h-screen flex flex-col bg-[#080c14] overflow-hidden">
      ...
      <header className="h-12 bg-[#0c1018] border-b border-[#1e2a42] flex items-center px-3 gap-0 shrink-0 z-10">
        ...
        <div className="flex items-center gap-2 shrink-0 pl-3 border-l border-[#1e2a42]">
          <div className="hidden sm:flex items-center gap-2 bg-[#111827] border border-[#1e2a42] rounded-lg px-2.5 py-1.5">
            <div className={`w-1.5 h-1.5 rounded-full transition-colors ${isConnected ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`} title={isConnected ? "Preços ao vivo" : "Atualizando…"} />
            <div>
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <aside className="w-14 bg-[#0c1018] border-r border-[#1e2a42] flex-col items-center py-3 gap-1 shrink-0 hidden md:flex">
          ...
          <div className="mt-auto flex flex-col gap-1">
            <Link href="/dashboard/profile">
              <SidebarBtn icon={<IconUser />} label="Perfil" active={false} onClick={() => {}} />
            </Link>
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0">
          <div className="h-11 bg-[#0c1018] border-b border-[#1e2a42] flex items-center px-4 gap-3 shrink-0">
            <div className="flex items-baseline gap-2">
              <span className={`text-lg font-bold font-mono price-flash transition-colors duration-300 ${
            ...
            <div className="hidden lg:flex items-center gap-1 ml-2">
              <span className="text-[9px] text-slate-700 bg-[#111827] border border-[#1e2a42] rounded px-1 py-0.5 font-mono">U</span>
              <span className="text-[9px] text-slate-700">cima</span>
              <span className="text-[9px] text-slate-700 bg-[#111827] border border-[#1e2a42] rounded px-1 py-0.5 font-mono ml-1">D</span>
              <span className="text-[9px] text-slate-700">baixo</span>
            </div>
          ...
          <div className="flex-1 min-h-0">
            {currentPrice > 0
              ? <TradingChart currentPrice={currentPrice} asset={selectedAsset} chartType={chartType} activeEntries={activeEntries} />
              : (
        ...
        <div className={`bg-[#0c1018] border-l border-[#1e2a42] flex flex-col shrink-0 w-[240px] ${showMobilePanel ? "absolute inset-0 z-30 w-full md:relative md:w-[240px]" : "hidden md:flex"}`}>
          ...
          {sidebarView === "trade" && (
            <div className="flex-1 overflow-y-auto flex flex-col min-h-0">
              <div className="p-3 space-y-3 shrink-0">
                ...
                {/* Duration */}
                <div>
                  <div className="text-[9px] font-semibold text-slate-600 uppercase tracking-widest mb-2">
                    Expiração <span className="text-slate-700 normal-case font-normal ml-1">teclas 1–4</span>
                  </div>
                  <div className="grid grid-cols-4 gap-1">
                    {DURATIONS.map((d, i) => (
                      <button key={d.value} onClick={() => setSelectedDuration(d.value)}
                        className={`py-1.5 rounded-lg text-xs font-semibold transition-all relative ${
                      ...
                      >
                        {d.label}
                        <span className="absolute -top-1 -right-1 text-[7px] text-slate-700 bg-[#0c1018] rounded px-0.5">{i+1}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-1 mt-1.5">
                    {([25, 50, 75] as number[]).map((pct) => (
                      <button key={pct}
                        onClick={() => setAmount(Math.max(1, Math.floor((balance ?? 0) * (pct / 100))))}
                        disabled={balance === null || balance < 1}
                        className="py-1 bg-[#0d1117] border border-[#1e2a42] rounded-lg text-[9px] text-slate-600 hover:text-slate-300 hover:border-[#2d4070] transition-colors disabled:opacity-30 font-semibold"
                      >{pct}%</button>
                    ))}
                    <button
                      onClick={() => setAmount(Math.max(1, Math.floor(balance ?? 0)))}
                      disabled={balance === null || balance < 1}
                      className="py-1 bg-[#0d1117] border border-amber-400/20 rounded-lg text-[9px] text-amber-600 hover:text-amber-400 hover:border-amber-400/40 transition-colors disabled:opacity-30 font-bold"
                    >MAX</button>
                  </div>
                ...
                <div className="bg-[#0d1117] border border-[#1e2a42] rounded-xl px-3 py-2.5">
                  <div className="text-[9px] text-slate-600 uppercase tracking-widest mb-1">Resultado</div>
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-[9px] text-emerald-600 mb-0.5">Se ganhar</div>
                      <div className="text-base font-bold text-emerald-400 font-mono">+${potentialWin.toFixed(2)}</div>
                    </div>
                    <div className="text-slate-700 text-lg font-light">|</div>
                    <div className="text-right">
                      <div className="text-[9px] text-rose-600 mb-0.5">Se perder</div>
                      <div className="text-base font-bold text-rose-400 font-mono">-${amount.toFixed(2)}</div>
                    </div>
                  </div>
                  <div className="text-[9px] text-slate-700 mt-1.5 font-mono text-center">
                    Payout {(PAYOUT_RATE * 100).toFixed(0)}% · ${amount.toFixed(2)} → ${(amount + potentialWin).toFixed(2)}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <span className="text-[8px] opacity-60 font-normal">[U]</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="text-[8px] opacity-60 font-normal">[D]</span>
                  </button>
                </div>
          ...
          <div className="px-3 py-2 border-t border-[#1e2a42] flex items-center gap-2 shrink-0">
            <Link href="/dashboard/profile" className="flex items-center gap-2 flex-1 min-w-0 hover:opacity-80 transition-opacity">
              ...
              <span className="text-[10px] text-slate-600 truncate">{session?.user?.name}</span>
            </Link>
      ...
      {/* Hidden ActiveTrades — settlement timers */}
