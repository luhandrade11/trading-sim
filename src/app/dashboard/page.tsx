...
import { PAYOUT_RATE, ASSETS, ASSET_TYPE, DURATIONS } from "@/lib/constants";
...
function SidebarBtn({
  ...
  return (
    <button
      onClick={onClick}
      title={label}
      className={`relative w-10 h-10 flex flex-col items-center justify-center rounded-xl transition-all gap-0.5 group ${
        active
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(p); clearInterval(d); if (staleTimerRef.current) clearTimeout(staleTimerRef.current); };
  ...
  const handleSettle = useCallback(async (id: string, exitPrice: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        await fetchUser();
        showNotif(settled.result === "WIN" ? `🏆 Ganhou! +$${settled.profit.toFixed(2)}` : `💸 Perdeu -$${settled.amount.toFixed(2)}`, settled.result === "WIN" ? "win" : "loss");
      } else {
        showNotif("Erro ao finalizar operação", "error");
      }
    } catch { showNotif("Erro de conexão ao finalizar", "error"); }
  }, [fetchUser, showNotif]);
  ...
  async function handleReset() {
    ...
    try {
      ...
      if (res.ok) { setBalance(1000); setTrades([]); settlingRef.current.clear(); showNotif("✓ Saldo resetado para $1.000", "win"); }
      else showNotif("Erro ao resetar saldo", "error");
    } catch { showNotif("Erro de conexão ao resetar", "error"); }
  ...
  return (
    <div className="h-screen flex flex-col bg-[#080c14] overflow-hidden">
      ...
      <header className="h-12 bg-[#0c1018] border-b border-[#1e2a42] flex items-center px-3 gap-0 shrink-0 z-10">
        ...
        <div className="flex items-stretch gap-0 overflow-x-auto scrollbar-hide flex-1 h-full">
          {ASSETS.map((asset) => {
            ...
            const isForex = ASSET_TYPE[asset] === "forex";
            return (
              <button
                key={asset}
                onClick={() => setSelectedAsset(asset)}
                className={`flex flex-col items-start justify-center px-3 h-full border-b-2 transition-all shrink-0 relative ${
                  isSelected
                    ? "border-amber-400 bg-[#111827]/60"
                    : "border-transparent hover:border-[#1e2a42] hover:bg-white/3"
                }`}
              >
                <div className="flex items-center gap-1.5">
                  <div className={`w-1.5 h-1.5 rounded-full ${isForex ? "bg-blue-400" : "bg-amber-400"}`} />
                  <span className={`text-[11px] font-semibold ${isSelected ? "text-white" : "text-slate-500"}`}>
                    {asset}
                  </span>
                </div>
                {p?.price ? (
                  <span className={`text-[9px] font-mono ${(p.change24h ?? 0) >= 0 ? "text-emerald-500" : "text-rose-500"} leading-none mt-0.5`}>
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        {/* ── SIDEBAR ── */}
        <aside className="w-14 bg-[#0c1018] border-r border-[#1e2a42] flex flex-col items-center py-3 gap-1 shrink-0 hidden md:flex">
          ...
          <div className="h-11 bg-[#0c1018] border-b border-[#1e2a42] flex items-center px-4 gap-3 shrink-0">
            <div className="flex items-baseline gap-2">
              <span className={`text-lg font-bold font-mono transition-colors duration-300 ${
                ...
                {currentPrice > 0 ? `$${formatPrice(currentPrice, selectedAsset)}` : <span className="text-slate-700 animate-pulse">——</span>}
              </span>
              {change24h !== 0 && (
            ...
            <div className="ml-auto flex items-center bg-[#111827] border border-[#1e2a42] rounded-lg p-0.5 gap-0.5">
              <button
                ...
                className={`px-2.5 py-1 rounded-md text-[10px] font-semibold transition-all ${chartType === "candle" ? "bg-amber-400/15 text-amber-400" : "text-slate-600 hover:text-slate-400"}`}
              >
                ◈ Candles
              </button>
              <button
                onClick={() => setChartType("line")}
                className={`px-2.5 py-1 rounded-md text-[10px] font-semibold transition-all ${chartType === "line" ? "bg-amber-400/15 text-amber-400" : "text-slate-600 hover:text-slate-400"}`}
              >
                ⟆ Linha
              </button>
            </div>
        ...
        <div className={`bg-[#0c1018] border-l border-[#1e2a42] flex flex-col shrink-0 w-[240px] ${showMobilePanel ? "absolute inset-0 z-30 w-full md:relative md:w-[240px]" : "hidden md:flex"}`}>
          ...
          <div className="px-3 pt-3 pb-2.5 border-b border-[#1e2a42] shrink-0">
            <div className="text-[9px] font-semibold text-slate-600 uppercase tracking-widest mb-1.5">Saldo Virtual</div>
              ...
              <div className="p-3 space-y-3 shrink-0">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {balance !== null && (
                      <span className="text-[9px] text-slate-700 font-mono">${balance.toFixed(2)}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-1 bg-[#0d1117] border border-[#1e2a42] rounded-xl overflow-hidden focus-within:border-amber-500/40">
                    <button
                      ...
                      className="w-8 h-9 text-slate-500 hover:text-white hover:bg-white/5 transition-colors text-sm font-bold shrink-0"
                    >−</button>
                    <input
                      type="number"
                      value={amount}
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    {currentPrice > 0 && (
                      <span className="text-[9px] opacity-70 font-mono">{formatPrice(currentPrice, selectedAsset)}</span>
                    )}
                  </button>
                  <button
              ...
              {stats.settled === 0 ? (
                ...
                <>
                  {[
                    ...
                    { label: "Total Investido", value: `$${stats.totalInvested.toFixed(2)}` },
                    ...
                    { label: "Streak Atual", value: stats.streak === 0 ? "—" : `${stats.streakType === "WIN" ? "🔥" : "❄️"} ${stats.streak}x`, color: stats.streakType === "WIN" ? "text-emerald-400" : "text-rose-400" },
                  ].map(({ label, value, color }) => (
      ...
      {/* ── HIDDEN ActiveTrades (handles settlement) ── */}
        ...
        <div className="h-10 bg-[#0c1018] flex items-center justify-between px-4 shrink-0">
          <div className="flex items-center gap-2">
            ...
            {activeTrades.length > 0 && (
              <span className="bg-amber-400/15 text-amber-400 text-[9px] font-bold px-1.5 py-0.5 rounded-full">
                {activeTrades.length}
              </span>
            )}
          </div>
          <button
            ...
            className="text-slate-600 hover:text-slate-300 transition-colors flex items-center gap-1 text-[10px]"
          >
